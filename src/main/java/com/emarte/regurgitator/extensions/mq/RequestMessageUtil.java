...
public class RequestMessageUtil {
	private static final Log log = getLog(RequestMessageUtil.class);

	public static Message applyMessageData(Message message, TextMessage jmsMessage) throws JMSException, RegurgitatorException {
	...
	public static void addMessageMetadata(Message message, TextMessage jmsMessage) throws JMSException, RegurgitatorException {
		...
		addStringParam(message, REQUEST_METADATA_CONTEXT, JMS_DESTINATION, jmsMessage.getJMSDestination().toString());
		...
		addStringParam(message, REQUEST_METADATA_CONTEXT, JMS_REPLY_TO, jmsMessage.getJMSReplyTo().toString());
